...
def login(request):
    if request.method == 'GET':
        ...
        # infostr = "<script>alert('sunck is a good man')</script>"
...
def verifycode(request):
    ...
    #构造字体对象
    font = ImageFont.truetype(r'C:\Windows\Fonts\Arial\ARIALN.TTF', 40)
    #构造字体颜色
    ...
    #（前端验证）将验证码的值写入cookie，以备前端浏览器验证验证码
    # response.set_cookie('verifycode',rand_str)

    # (服务器验证———15行判断验证)存入session，用于做进一步验证
