...
from axf.models import SliderShow, Product, MainDescription, Categorie, ChildCategorie,Addresses,User,Cart,Order
...
def mine(request):
    phone = request.session.get('phone',default='Loading')
...
def verifycode(request):
    ...
    # 在POST表单提交前，此处mobile是通过ajax请求页面获取的
    ...
    return JsonResponse({'data':random_str})
...
def createInfo(request):
    ...
    if request.path == '/createInfo/' :
        ...
        if request.method == 'GET':
            infos = Addresses.objects.filter(user=request.user)
            return render(request, 'mine/address.html', {'infos': infos,"fromPath":fromPath})

def append(request):
    if request.method == 'GET':
        return render(request,'mine/append.html')
    if request.method == 'POST':
        name = request.POST.get('person')
        sex = request.POST.get('sex')
        phoneNum = request.POST.get('phonecode')
        city = request.POST.get('city')
        county = request.POST.get('county')
        street = request.POST.get('rowd')

        token = request.COOKIES.get('token')
        user = User.objects.get(tokenValue=token)

        add = Addresses.create(name,sex,phoneNum,None,city,county,street,None,user)
        add.save()
        response = redirect('/createInfo/')
        return response
...
def update(request):
    ...
    if request.method == 'POST':
        token = request.COOKIES.get('token')
        user = User.objects.get(tokenValue=token)
        put = Addresses.objects.filter(user=user)
        put.delete()
